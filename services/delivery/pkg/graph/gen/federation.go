...
func (ec *executionContext) resolveEntity(
	...
	case "Delivery":
		...
		case "findDeliveryByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findDeliveryByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindDeliveryByID(ctx, id0)
			if err != nil {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Delivery", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
