...
//		GroupBy(orderitem.FieldProductName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.OrderItem.Query().
//		Select(orderitem.FieldProductName).
//		Scan(ctx, &v)
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	for i := range oiq.loadTotal {
		if err := oiq.loadTotal[i](ctx, nodes); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Order)) error {
	...
	for i := range nodes {
		if nodes[i].order_order_item == nil {
			continue
		}
		fk := *nodes[i].order_order_item
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_order_item" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
