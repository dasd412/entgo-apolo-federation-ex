...
func (*OrderItem) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case orderitem.ForeignKeys[0]: // order_order_item
			values[i] = new(sql.NullInt64)
		default:
...
func (oi *OrderItem) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case orderitem.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field order_order_item", value)
			} else if value.Valid {
				oi.order_order_item = new(int)
				*oi.order_order_item = int(value.Int64)
			}
		default:
